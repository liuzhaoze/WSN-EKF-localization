...
def trilateration_localization(position, distance):
    '''
        ...
        for each in uwb_wsn_data_:
            if each.node_id not in localized:
                ...
                sensor_position[each.node_id] = [
                    trilateration_localization(position, distance)]
                # !!!!!!!!!!!! MODIFIED HERE !!!!!!!!!!!!
                # trilateration only use anchor pose, doesn't use the pose of localized unknown sensor
                # so the code below should be "comment", and the number of localized unknown sensor is fewer than maximum_likelihood_estimation
                # localized.add(each.node_id)
...
if __name__ == '__main__':
    rospy.init_node('trilateration_localization', anonymous=True)

    global anchor_pos
    anchor_pos = get_sensor_pos('anchor', 3)

    rospy.Subscriber('uwb_wsn_data_topic', uwb_wsn_data, uwb_wsn_data_update)
    rospy.Subscriber('ekf_localization_data',
                     uwb_wsn_slam_data, ekf_data_update)

    rospy.Service('trilateration_localization_server',
                  estimation_result, request_handler)
    rospy.loginfo('Service: trilateration_localization_server ready.')
